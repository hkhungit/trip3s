	...
	var getOptions = function(options) {
	    ...
	    acoCreateSchedule = function(optionsSource) {
	        ...
	        var _antResult = {},
	            ...
	            _antBestSchedule = [],
	            _antBestMoney = 0,
	            _Places = _saveArray(optionsSource.placeLists),
	            _currentLoop = optionsSource.antCurrLoop,
	            _maxLoop = optionsSource.antMaxLoop,
	            _antNumber = optionsSource.antNumber,
	            _antCurrDistance = 0,
	            _antCurrDuration = 0,
	            _antCurrMoney = 0,
	            timeCurrent = optionsSource.timeStart;
	            ...
	            for (var ant = 0; ant < _antNumber; ant++) {
	                ...
	                for (var i = 0; i < _Places.length; i++) {
	                    ...
	                    for (var day = 1; day <= parseInt(optionsSource.dayNumber); day++) {
	                        ...
	                        if ((_tmpRs.money > parseFloat(optionsSource.schedules["Day_" + day].moneyNumber) && parseFloat(optionsSource.schedules["Day_" + day].moneyNumber) > 0) || _tmpRs.notPlaceLists.length > 0 || _tmpRs.placeLists.length < 1 || parseFloat(_tmpRs.timeCurrent) > parseFloat(optionsSource.schedules["Day_" + day].timeEnd)) {
	                            if (day == parseInt(optionsSource.dayNumber)) {
	                                break;
	                            };
	                            continue;
	    ...
	    acoAlgorithm1 = function(options) {
	        var _Places = _saveArray(options.placeLists),
	            timeCurrent = 0,
	            distance = 0,
	            _antCurrMoney = 0;
	        var _checkTmp = _checkPlace(options, {
	            place1: options.placeBegin,
	            place2: _Places[0]
	        }, options.timeStart);
	        ...
	        var _checkTmp2 = _checkPlace(options, {
	            place1: _Places[0],
	            place2: options.placeEnd
	        }, timeCurrent);
	    ...
	    acoAlgorithm = function(options) {
	        ...
	        var _antResult = {},
	            ...
	            _antBestDuration = 0,
	            _antBestMoney = 0,
	            _Places = _saveArray(options.placeLists),
	            _currentLoop = options.antCurrLoop,
	            _maxLoop = options.antMaxLoop,
	            _antNumber = options.antNumber,
	            _antCurrDistance = 0,
	            _antCurrDuration = 0,
	            _antCurrMoney = 0,
	            ...
	            for (var ant = 0; ant < _antNumber; ant++) {
	                ...
	                for (var i = 0; i < _Places.length; i++) {
	                    ...
	                    if (i == 0) {
	                        var _checkTmp = _checkPlace(options, {
	                            place1: options.placeBegin,
	                            place2: _Places[options.antSchedule[i]]
	                        }, timeCurrent);
	                        ...
	                        var _checkTmp = _checkPlace(options, {
	                            place1: _Places[options.antSchedule[i]],
	                            place2: options.placeEnd
	                        }, timeCurrent);
	                        ...
	                        var _checkTmp = _checkPlace(options, {
	                            place1: _Places[options.antSchedule[i - 1]],
	                            place2: _Places[options.antSchedule[i]]
	                        }, timeCurrent);
	    ...
	    _SaveArraySchedule = function(options) {
	        var placeLists = [],
	            ...
	            placeIds = [],
	            notplaceIds = [],
	            places = options.placeLists,
	            schedule = [],
	            bestSchedule, notSchedule = [];
	    ...
	    _checkPlace = function(options, places, timeStart) {
	        var antVector = options.antVectorDistance,
	            nextTime = 0,
	            nextDistance = 0,
	            moneyPlace = 0,
	        ...
	        if ((_decryptTime + parseFloat(places.place2.place_time1)) > encryptTime(places.place2.place_close)) {
	            places.place2.place_time = encryptTime(places.place2.place_close) - _decryptTime;
	        };

	        places.place2.place_come = _decryptTime;
	        places.place1.next_time = nextTime;
	        places.place1.next_distance = nextDistance;
	        places.place2.place_time = places.place2.place_time1;
	    ...
	    getArrayDistances = function(options, arrDistances) {
	        var obj = window.javo_map_box_func,
	            _places = _saveArray(options.placeLists),
	            time = 101;
	        if (typeof options.placeBegin != 'undefined') {
