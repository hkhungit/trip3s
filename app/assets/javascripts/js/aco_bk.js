		...
		for (var m = 0; m < antNumber; m++) {
			...
			for (var i = 0; i < options.placeLists.length; i++) {
				...
				if (i == 0) {
					...
					_Places[antSchedule[i]].place_come		= decrypt_time(next_time);
					...
					antCurrDistance 	+= next_distance ;
					antBestDuration1 	+= next_time
					_Places[antSchedule[i]].place_come		= decrypt_time(antBestDuration1);
					antBestDuration1 	+= parseFloat(_Places[antSchedule[i]].place_time);
				};
