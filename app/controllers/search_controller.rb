class SearchController < ApplicationController
  ...
  def index
   ...
   sql = " select A.id,A.post_title, A.post_review,A.post_view,B.place_choice,B.place_address,A.post_url,A.post_thumbnail from posts A,places B WHERE A.post_title like '%#{params[:search]}%' and A.id=B.post_id and A.id in (select id FROM posts ORDER by post_review DESC) ORDER by B.place_choice DESC"
